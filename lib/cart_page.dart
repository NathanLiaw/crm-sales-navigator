...
class _CartPage extends State<CartPage> {
  ...
  Future<List<Map<String, dynamic>>> getProduct() async {
    try {
      final conn = await connectToDatabase();
      String query =
          'SELECT id, description, price_by_uom FROM product WHERE status = 1';

      final results = await conn.query(query);
      print("test");
      await conn.close();

      List<Map<String, dynamic>> products = results.map((row) {
        return {
          'id': row['id'] as int?,
          'description': row['description'],
          'price_by_uom': row['price_by_uom'].toString(),
        };
      }).toList();
              ...
              if (cartItems.isEmpty)
                ...
                Column(
                  children: List.generate(cartItems.length, (index) {
                    ...
                    // Retrieve product details from the products list
                    final product = products[index];
                    final productId = product['id'] as int;
                    final itemDescription = product['description'];
                    final priceByUom = product['price'].toString();
                      ...
                      child: GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => ItemScreen(productId: item.productId, productName: item.productName, priceByUom: priceByUom, itemAssetName: 'asset/${itemPhotos[0]}', itemDescription: itemDescription,),
                            ),
                          );
                        },
                        child: Card(
