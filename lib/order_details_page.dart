...
class _OrderDetailsPageState extends State<OrderDetailsPage> {
  ...
  Future<void> voidOrder() async {
    ...
    try {
      ...
      // Query cart table for session
      final cartResults = await readData(
        conn,
        'cart',
        'id = ${widget.cartID}',
        '',
      ...
      if (cartResults.isEmpty) {
        print('Cart not found for ID: ${widget.cartID}');
        return;
      }

      final session = cartResults.first['session'] as String;
      // Query the cart_item table based on session
      final cartItemResults = await readData(
        conn,
        'cart_item',
        'session = "$session"',
